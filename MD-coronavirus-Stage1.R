...
# Read case data from the counties
...
qS <- ggplot(dt, aes(x = Date, y = Cases, group = County)) +
  ...
  #geom_line(data = subset(dt, County == "AnneArundel"), aes(x = Date, y = Cases)) +
...
countiesList <- colnames(datCountyDeaths)
counties2remove <- countiesList[-c(1)] # we'll keep date and remove all counties (incl. DnA) - to keep the delta ("D") cols

# Find daily variation (I'm sure there is a faster way)
datCountyDeaths <- datCountyDeaths %>%
  mutate(DAllegany = Allegany - lag(Allegany, default = 0)) %>%
  mutate(DAnneArundel = AnneArundel - lag(AnneArundel, default = 0)) %>%
  mutate(DBaltimoreCity = BaltimoreCity - lag(BaltimoreCity, default = 0)) %>%
  mutate(DBaltimoreCounty = BaltimoreCounty - lag(BaltimoreCounty, default = 0)) %>%
...
# dat will have names of counties but with daily delta only
dat <- datCountyDeaths[ , -which(names(datCountyDeaths) %in% counties2remove)]
colnames(dat) <- countiesList[1:length(dat)]

# datA will have moving average of daily new cases
dat[mapply(is.na, dat)] <- 0 # set NAs to 0 might induce some change when start deaths
matA <- apply(dat[,2:length(dat)], 2, SMA, n=7)
datA <- cbind(dat$Date, as.data.frame(matA)) # get back to dataframe
colnames(datA) <- countiesList[1:length(datA)]
...
dt <- pivot_longer(data = datA, cols = cols2pivot, names_to = "County", values_to = "Deaths", values_drop_na = TRUE)
dt$Date <- as.Date(sprintf("%d",dt$Date), "%y%m%d")
...
p <- ggplot(dt, aes(x = Date, y = Deaths, group = County)) +
  ...
  labs(title = "7-Day average daily new COVID-19-confirmed deaths in Maryland counties, USA (2020)",
       x = "Date",
       y = "7-Day average daily number of deaths")
...
dt <- pivot_longer(data = datB, cols = cols2pivot, names_to = "County", values_to = "Deaths", values_drop_na = TRUE)

# Add group of Phase 1 the county is in
dt <- merge(dt, datStage1, by = "County")

dt[mapply(is.infinite, dt)] <- NA
...
dtS <- dt %>%
  ...
  summarise(
    mean = mean(Deaths),
    sd = sd(Deaths),
    sem = sd(Deaths) / sqrt(length(Deaths))
  )
...
#   scale_color_manual(values=c("#e10304", "#0db104", "#9eb4cc")) +
#   theme_linedraw() +
#   labs(title = "Average deaths relative to 1st day of Stage 1 (May 15, 2020)",
#        x = "Date",
...
qS <- ggplot(dt, aes(x = Date, y = Deaths, group = County)) +
  ...
  stat_summary(
    mapping = aes(x = Date, y = Deaths, group = Status, fill = Status),
    fun = "mean",
    geom = "ribbon",
    fun.min = "lci",
    fun.max = "uci",
